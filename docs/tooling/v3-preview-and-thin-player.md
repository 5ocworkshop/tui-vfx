...
<!-- <CLOG>0.2.0: record the packaged tui-vfx-player summary CLI as the first thin-player slice.</CLOG> -->
...
| Packaged thin player | `/usr/projects/tui-vfx-recipes/tools/tui-vfx-player` | Stable `tui-vfx-player` package with text and `--json` summary modes over the existing preview/cutover APIs. |

## D2 packaged thin-player slice

The first packaged thin-player slice now exists in `tui-vfx-recipes` as the
`tui-vfx-player` workspace package. It intentionally stays small: it loads a
...
```text
cargo run -q -p tui-vfx-player -- recipes/debug_recipes/content/content_slide_shift.json
cargo run -q -p tui-vfx-player -- recipes/debug_recipes/content/content_slide_shift.json --json
```

The broader tooling family remains split by responsibility:

```text
tui-vfx-player --json        -> stable packaged recipe playback summary
...
4. do not add `gtd-movie` timeline/composer semantics to `tui-vfx-player`,
