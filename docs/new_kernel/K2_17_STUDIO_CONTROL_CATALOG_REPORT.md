# K2.17 Studio Control Catalog Report

The `control-catalog` CLI derives source/effect controls from descriptor packs and can annotate controls used by a specific recipe. Current descriptor catalog exposes 367 controls: 16 source controls and 351 effect controls.
Gradient inputs use the durable `gradientEditor` control kind; implementation maturity is not encoded in machine-readable control-kind names.
