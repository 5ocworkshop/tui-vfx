...
<!-- <CLOG>1.15.0: mark focused-row-gradient style.spatial as compositor shader-layer owned and record style-stage deletion.
1.14.0: mark shader.diffusion as compositor shader-layer owned and record style-stage deletion.
...
| `style.spatial` focused_row_gradient | `ShaderLayerSpec { shader: SpatialShaderType::FocusedRowGradient(FocusedRowGradientShader), region: StyleRegion::Cell { ... } }` | Done for current cell-position recipe: lowerer emits a compositor shader layer with cell-scope bindings preserved in `StyleRegion`, and backend `NativeStyleStage::SpatialFocusedRowGradient` plus its style helper were deleted. |
