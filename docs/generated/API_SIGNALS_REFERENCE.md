...
<!-- <FILE>docs/generated/API_SIGNALS_REFERENCE.md</FILE> - <DESC>Engine / direct-API signal reference for mixed-signals primitives</DESC> -->
<!-- <VERS>VERSION: 0.2.0</VERS> -->

# Engine / Direct-API Signals Reference

Reference for `mixed_signals::*` primitives reached by direct API construction in engine code. The **Core 12** list at the top is the recommended starting point. Per-family sections below document every signal primitive available through `SignalSpec` JSON deserialization and through direct construction.
...
| 8 | **spring** | Damped spring harmonic motion solver. | `spring` |
...
```jsonc
{"type": "spring", "mass": 1.0, "stiffness": 10.0, "damping": 1.0, "v0": 0.0, "x0": 1.0}
```

**Recipe hint:** Damped harmonic motion (`DampedSpring`, exposed as the `spring`
...
```jsonc
{"type": "attractor", "target_x": 0.0, "target_y": 0.0, "strength": 1.0, "falloff": 2.0}
```
...
```jsonc
{"type": "bounce", "start_height": 1.0, "ground_height": 0.0, "gravity": 9.81, "restitution": 0.7}
```
...
```jsonc
{"type": "orbit", "center_x": 0.0, "center_y": 0.0, "radius": 1.0, "angular_velocity": 1.0, "start_phase": 0.0}
```
...
```jsonc
{"type": "pendulum", "length": 1.0, "gravity": 9.81, "theta0": 0.5, "damping": 0.1}
```
...
```jsonc
{"type": "projectile", "start_x": 0.0, "start_y": 1.0, "v0_x": 1.0, "v0_y": 1.0, "gravity": 9.81, "ground_y": 0.0}
```
