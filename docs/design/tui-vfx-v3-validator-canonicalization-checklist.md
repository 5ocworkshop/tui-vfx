...
| VC-06 | Contract discovery validation       | COMPLETE_INITIAL | `requires_tokens`, `requires_bindings`, `requires_assets`, and `requires_primitives` now have normalized declaration-shape checks; `pipeline-validator --dump-normalized` includes `contract_usage` for discovered bindings/template refs plus undeclared usage sets |
...
4. **Hard-error promotion gate remains follow-on**
   - missing declaration usage is reported, not globally fatal, because existing
     binding-focused debug fixtures still need corpus compatibility updates.

## Next slices after VC-06

1. **VC-06 strict-mode/corpus compatibility:** add `requires_bindings` declarations to the current binding-focused V3 debug corpus, then promote undeclared usage to an explicit strict-mode gate.
2. **VC-02/VC-05 strict diagnostics:** unresolved region references, impossible
