...
<!-- <WCTX>Tracks the first code-facing work package after schema/catalog/lowering/IR/validator planning. Updated now that the initial V3 scaffolding code has landed in tui-vfx-recipes and has its first compile/test signal.</WCTX> -->
<!-- <CLOG>0.3.0: record initial verification signal for FS-01/FS-02 after the V3 scaffold compiled under `cargo check --lib` and the focused parse unit test passed.
