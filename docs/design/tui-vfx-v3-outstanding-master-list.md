...
<!-- <CLOG>0.14.22: record the V3 border custom-chars/frame source rendering slice and remaining border parity follow-up.</CLOG> -->
...
| V3-BORDER01 | Border parity and schema hardening | Complete initial / V2 role-map follow-up remains | `single` now parses as the line-drawn plain border, V3 trim precedence has tests for omitted trim and explicit `trim: none`, and `tui-vfx-recipes` commit `d80bb7d` makes root V3 `custom_chars` and `frame` render into border-role source cells. Remaining follow-up: decide whether to build semantic border/text role maps for V2 rendered sources or document that V2 role-targeting remains partial during migration. |
...
| V3-BORDER01-FOLLOW | V2 border role-map parity decision | V3 root source now supports `single`, titles, `custom_chars`, and `frame` with border roles. Remaining border audit item is V2 source role truth: either construct semantic border/text role maps for V2 rendered sources or document/lint that V2 role-scoped border effects are partial until recipes migrate to V3. |
...
| V3-TOOL01 | Work down Chapter 100 release checklist | `tui-vfx-horseman` is the packaged headless summary/corpus command, and debug QC now accepts multi-sampler, none-mask, and exit-phase FadeToCanvas fixtures. Remaining work is CI adoption, broader trace/probe parity, and release-gate policy closure. |
...
<!-- <VERS>END OF VERSION: 0.14.19</VERS> -->
