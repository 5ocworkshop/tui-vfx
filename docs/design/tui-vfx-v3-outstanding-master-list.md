...
<!-- <CLOG>0.14.21: record the shadow edge-crossing fix and add border parity audit follow-ups.</CLOG> -->
...
| V3-BORDER01 | Border parity and schema hardening | Outstanding / audit returned | Border audit found P0/P1 V2↔V3 deltas: `single` is not a supported border type and can silently render no border, V3 trim defaults may preserve when V2/default intent is vanishing-edge, V3 root border support is narrower than V2 for custom chars/frame/title, and V2 source role maps lack border truth. Next action: implement `single` alias or diagnostic, lock trim precedence/defaults, and add border-role regression fixtures. |
...
| V3-BORDER01 | Fix border parity and diagnostics | Implement the returned border audit: `single` naming support/diagnostics, V3 trim default/precedence tests, border role regression coverage, and explicit lint/docs for V2-only border subfields (`custom_chars`, `frame`, `title`) until implemented. |
