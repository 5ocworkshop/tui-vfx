...
| V3-META01 | Implement accepted metadata policy | Schema/docs/validator support optional metadata; debug/reference fixtures warn when `description` or `metadata.expected_visual` do not explain what the viewer should see. |
| V3-TIME01 | Apply accepted distributed-timing policy | Authoring docs and schema/rustdoc explain lifecycle timing, `mixed-signals` temporal signals, effect-local timing, and why no universal Timer primitive exists in V3. |
