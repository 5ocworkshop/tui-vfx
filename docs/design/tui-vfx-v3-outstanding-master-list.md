...
<!-- <VERS>VERSION: 0.14.29</VERS> -->
<!-- <WCTX>Add V3-LOOPBACK-NN01 entry capturing the eight non-numeric binding-loopback holdouts surfaced by the L5 loopback_fires_sweep tool — bool/string/color bindings cannot use the numeric-only v1 `loopback: {signal: ...}` shape and stay on the legacy `default:` literal lift, so the L3 visibility badge fires every frame for each.</WCTX> -->
<!-- <CLOG>Add V3-LOOPBACK-NN01 row above V3-R99 listing the eight non-numeric holdouts and the per-kind signal-vocabulary extension work needed to take them off the badge.</CLOG> -->
...
| V3-PROCRUNTIME01 | Procedural source host-injection (`ShaderRuntimeParams` in `ProceduralCtx`) | **Release-blocking** / outstanding | Procedurals today consume only static recipe params + clock + signal_ctx; `ProceduralCtx` (at `crates/tui-vfx-recipes/src/scene/procedural/types/cls_procedural_ctx.rs`) does **not** carry `&ShaderRuntimeParams`, so live progress meters / counters / scrubber-fed procedural sources cannot be wired by host code. Phase P0 work: thread `&ShaderRuntimeParams` through `ProceduralCtx` and update affected source impls (start with `cls_subcell_shape_atlas` and the live-progress-meter target). Independently shippable as P0-narrow (host-injection only) before P0-full (which adds binding-loopback support per `completed/tui-vfx-binding-loopback.md` section 9 once that side quest's L1+L2 ship). Driver: live progress-meter procedurals are release-blocking per owner. Two-cut split documented in `docs/design/completed/tui-vfx-binding-loopback.md` section 12 and section 9. |
| V3-LOOPBACK-NN01 | Non-numeric loopback signal vocabulary (Bool / String / Color) | Outstanding / post-stability | The L2 `loopback: {signal: ...}` authoring shape is **numeric-only** in v1 (drives `BindingKind::U16` / `BindingKind::F32`). Eight bindings across the corpus are non-numeric and therefore stay on the legacy `default:` literal lift — they satisfy the Intention 37 strict-contracts gate but cannot animate in preview, so the L3 visibility badge fires every frame for each. To grow `LoopbackValue` (at `crates/tui-vfx-recipes/src/loopback/enum_loopback_value.rs`) the work is: (a) define what a non-numeric "signal" means per kind (bool: square/pulse from a numeric driver? string: enumerated step from an index? color: lerp between RGB pairs?); (b) extend `V3LoopbackValue::Signal(...)` parsing to accept the new shapes; (c) extend `evaluate_loopback` to dispatch per-kind. Driver: the eight holdout bindings listed below render as static defaults in preview, masking what they're actually driving (visibility toggles, asset selection, hover direction, color choice). Holdout inventory: `madeira_flag.full_scene` (`fireworks_enabled`, `show_hint` — bool); `debug.filter.fade_to_canvas_canvas_color_binding` (`current_terminal_bg` — color); `debug.scene.authoring_ladder_procedural_spinner_binding` (`show_spinner` — bool); `debug.scene.image_source_bindable` (`selected_logo` — string); `debug.scene.layer_visibility_binding_io` (`show_detail` — bool); `debug.shader.composition.glisten_band_direction_blend_binding` (`hover_direction` — string); `debug.shader.composition.highlighter_runtime_bindings` (`hover_direction` — string). Run `python3 tools/loopback_fires_sweep.py` against the corpus to regenerate this list any time. |
...
| V3-VALIDATOR02 | Close rules/probe contract gap for V3 payloads and scopes | Rules-only validation can currently miss payload-field and channel-scope mismatches that `--probe` catches. Decide whether to lift more runtime contract checks into rules/strict-contracts, or emit explicit validator guidance that probe is the authoritative runtime compatibility gate; add tests for representative failures such as `highlighter.intensity`, unsupported `glisten_sweep.color`, and channel-scoped AO/barber-pole/charset-noise. |
| V3-SCOPE02 | Resolve channel-scope support for AO/barber-pole/charset-noise | Recipe authors repeatedly try `channel:background` with `ambient_occlusion`-lowered overlays, `barber_pole`, and `charset_noise`; today those need `all` or concrete region containment. Either implement true channel-scope support where semantically valid, or add earlier diagnostics and authoring fixtures that prove the intended containment pattern. |
...
| V3-SUBCELL-SHAPES | Complete initial | `tui-vfx-recipes` commit `4836586`: adds the `subcell_shape_atlas` procedural source with FractionalInsetRect, SubcellFrame, SteppedParallelogram, BrailleRoundedRect, and QuadrantCornerSculpt; adds primitive-first debug recipes under `recipes/debug_recipes/subcell_shapes/` plus procedural/QC/probe validation. |
| V3-M01-HARNESS | Complete hard-code / owner-audit blocked for complete-final | `tui-vfx-recipes` commit `680e5d6`: VC-09 migration-equivalence mismatch evidence now emits structured output/probe diagnostics with fingerprints and changed evidence-surface keys; exact matches emit empty diagnostics. Harness tests passed, and complete-final corpus closure remains blocked only on `OWNER-RECIPE-AUDIT` / `V3-M02`. |
| V3-SCHED01-CODE | Complete initial | `tui-vfx-recipes` commits `19e9aca`, `a66a21d`, and `17c1eec`: scheduler batching semantics are guarded for sequence feed-forward, parallel snapshot isolation, duplicate hint-output rejection, batch-safe hashes, and overlap-conflict fixtures. Fresh `cargo test -q -p tui-vfx-recipes --test test_v3_scheduler_batching_audit` and `cargo test -q -p tui-vfx-recipes --lib scheduler_` passed. |
| V3-BIND01-CODE | Complete initial | `tui-vfx-recipes` commit `3c358bc`: runtime binding/scalar sampling now covers non-spatial style replay without collapsing spatial fields. Fresh isolated checks passed for `tui-vfx-recipes`, `tui-vfx-compositor` prepared filters, and `tui-vfx-style` binding tests. Broader mold-linked recipe tests remain an environment/toolchain risk, not a known code blocker. |
| V3-EDGE01-CODE | Complete initial | `tui-vfx-recipes` commits `7005b7e` and `a66a21d`: scene-layer edge crossing now follows source-border roles, diagonal/two-edge shadow-shifted border blanking is regression-tested, and fresh headless verification passed for the focused edge test plus `cargo check -q -p tui-vfx-recipes --lib`. |
| V3-SPATIAL02-CODE | Complete initial | `tui-vfx-recipes` commit `3c358bc`: spatial-field payloads are preserved for bridge/showcase consumers while non-spatial style hints are sampled at replay time; runtime overrides and style-family replay tests landed, and Madeira strict-contract validation passed. |
...
| V3-C02-FOLLOW | Complete initial | `tui-vfx-recipes` commit `fe48053`: recipe normalization now canonicalizes named-factory/compositional shader payload pairs (`gradient_overlay`, `fractional_stripe_overlay`, and `colored_overlay` perimeter/edge/radial patterns) before normalized IR leaves the recipe layer; table-driven equivalence tests also prove each canonicalized form reaches the executable V3 style-family seam. |
