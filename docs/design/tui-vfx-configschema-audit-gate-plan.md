...
<!-- <WCTX>2026-04-27: third revision after discovering that v2.0.0 read the dead OFPF-prefixed files in tui-vfx-core-macros/src/ rather than the live macro at lib.rs. Full reads of the live macro flipped multiple Phase 2 reclassification calls — derive is more capable than v2.0.0 claimed; several baseline entries are real migration candidates. Also surfaced an audit scanner bug (qualified-path blind spot) and a workspace-localized lib.rs hygiene regression (handled in a separate packet). Per the user directive: complete and thorough is ALWAYS the bar.</WCTX> -->
<!-- <CLOG>3.0.0: full third revision after end-to-end reads of every load-bearing file. KEY CORRECTIONS over v2.0.0: (a) the live derive macro at `tui-vfx-core-macros/src/lib.rs` (659 lines) is much more capable than the dead OFPF siblings v2.0.0 read — it emits per-variant descriptions from `///` comments, json_value from `#[serde(rename)]`/`rename_all`, tag_field from `#[serde(tag)]`, and field default/min/max from `#[config(...)]`; (b) consequently PathType/RoleTag/Color/StyleRegion/Never are real derive candidates, not "investigate" candidates; (c) the audit scanner has a fully-qualified-path coverage gap missing 4 impls in `cls_cursor_shader.rs` (×2) and `enum_vfx_cursor_behavior.rs` (×2); (d) StyleRegion's baseline `note` is doubly wrong (claimed dependency edge that exists, claimed cross-crate-trait-dep when RoleTag has its own ConfigSchema); (e) RoleTag derivability is gated on InternedRoleName needing a ConfigSchema impl. Process note: v1.0.0 prescribed reinventing shipped infrastructure; v2.0.0 corrected that but read dead code; v3.0.0 read live code end-to-end. Each iteration's CLOG names the failure mode the next pass corrected.</CLOG> -->
...
1. **Macro crate cleanup packet.** `tui-vfx-core-macros/src/lib.rs` is 659 lines of inline logic that should live in OFPF-prefixed siblings. The dead `fnc_*.rs` files in the same dir are an abandoned earlier refactor. Cleanup plan: see `docs/design/tui-vfx-core-macros-lib-rs-cleanup-plan.md` (companion file).
