...
<!-- <CLOG>1.5.3: record that the browser demo now understands the same direct rendered V3 snapshot subset as play_recipe, even though the full native animated V3 preview path is still outstanding.
...
- **Customer-facing preview/demo seam**
  ...
  - `demo` now understands that same direct rendered V3 snapshot subset for visual browsing, but it still uses the older preview-manager path for legacy recipes and is not yet a fully native animated V3 preview surface
...
<!-- <VERS>END OF VERSION: 1.5.3</VERS> -->
