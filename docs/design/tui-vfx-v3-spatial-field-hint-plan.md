...
- content-before-pipeline composition ✅ (`typewriter` source generation feeding a sourced-output filter/shader chain)
- binding-gated scene-layer I/O ✅ (`visibility.predicate` from `requires_bindings`, then local sourced-output chain)
- parallel-join I/O ✅ (`Parallel` producer output consumed by a later `Sequence` child, including a time-varying filter-to-mask chain)
- SCHED-03 batching-readiness ✅ (field-hint joins are classified as
...
A sixth proof covers content composition: top-level `content.effect =
typewriter` resolves source text before `pipeline.step`, then the downstream
sequence runs the same sourced-output filter-to-shader chain over those cells.
This proves content-to-pipeline composition without inventing a content-specific
hint channel.
