...
<!-- <WCTX>Phase E of TransformContext implementation plan: close Slice 6.6 in the cycle plan and bump to v0.8.0.</WCTX> -->
<!-- <CLOG>0.8.0: close Slice 6.6 — TransformContext + binding-form font runtime threading shipped (Phases A–D commits e7fbd48, 81ae71a, c4b38de, 3180a1c, 2f0dc14).</CLOG> -->
...
| 6.6 | Thread `ShaderRuntimeParams` to `TextTransformer` | **Done** | `TransformContext` struct + `TextTransformer::transform` signature change + all 15 transformer impls + Odometer reads `runtime_params` for font binding resolution. Phase A (tui-vfx) `e7fbd48`; Phase B (gt-design) `81ae71a`; Phase A.10 follow-up (tui-vfx-recipes runtime callers) `c4b38de`; Phase C (tui-vfx, Odometer) `3180a1c`; Phase D (tui-vfx-recipes, recipe) `2f0dc14`. |
...
| 6.6 | Thread `ShaderRuntimeParams` through `TextTransformer` | **Done.** `TransformContext` struct bundles `SignalContext` + `ShaderRuntimeParams`; `TextTransformer::transform` now takes `&TransformContext`; `cls_odometer.rs` reads `runtime_params` to resolve Binding-form font references. All 15 transformer impls updated. Commits: Phase A `e7fbd48`, Phase B `81ae71a`, Phase A.10 `c4b38de`, Phase C `3180a1c`, Phase D `2f0dc14`. |
