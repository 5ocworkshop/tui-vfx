...
<!-- <CLOG>1.28.0: record that the legacy flat SpatialShaderType surface now exposes the grouped V3 family seam directly, and key downstream callers consume that method instead of importing the free lowering helper ad hoc.
1.27.0: record that the direct compiled-V3 visual bridge now applies non-spatial style effects in the preferred playback surfaces instead of remaining spatial-only.
...
1.21.0: record broadened grouped-V3-to-legacy lowering coverage across representative primitive and composed family clusters.
1.20.0: record that direct probe pipeline inventory now carries grouped V3 spatial shader family labels.
1.19.0: record broadened grouped-spatial runtime equivalence coverage across representative primitive and composed families in spec-driven rendering tests.
1.18.0: record that render_pipeline_with_spec now consumes grouped-V3-aware ShaderWithRegion construction instead of rebuilding spec shader layers ad hoc.
...
1.11.0: record that unified probe pipeline inventory now carries grouped V3 style-effect family labels for active non-spatial style effects.
1.10.0: record that validator/probe stage reporting now carries grouped V3 style-effect family labels from runtime style interpolation.
...
- [~] any additional cross-family refactors needed once runtime wiring begins
  ...
  - [x] `SpatialShaderType::v3_spatial_shader_family()` / `v3_family_label()` expose that grouped seam directly on the legacy flat surface
  - [~] first runtime-facing consumers
    ...
    - [x] unified probe pipeline inventory carries grouped effect-family labels for active non-spatial style effects
    - [x] operational analysis + sqlite-backed probe analysis rows carry grouped style-effect family labels
    ...
    - [x] representative primitive and composed family equivalence coverage exists for grouped-V3 spec-driven rendering
    - [x] direct probe pipeline inventory carries grouped V3 spatial shader family labels
    - [x] grouped-V3-to-legacy lowering coverage spans representative primitive and composed family clusters
