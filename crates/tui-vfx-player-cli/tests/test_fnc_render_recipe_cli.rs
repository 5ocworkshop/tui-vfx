...
// <CLOG>0.41.0: MINOR — lock focused-row-gradient style.spatial compositor shader-layer lowering.
// 0.40.0: MINOR — lock shader.diffusion compositor shader-layer lowering.
...
fn test_fnc_cli_lowers_focused_row_gradient_spatial_style_to_compositor_shader_layer_json() {
    let report = player_cli_json(
        vec![
            str_arg("render-backend"),
            str_arg("--recipe"),
            recipe_path("styles/style_cell_position_binding.json"),
            str_arg("--descriptor-pack"),
            descriptor_pack_path(),
            str_arg("--backend"),
            str_arg("compositor"),
        ...
        "render-backend native focused-row-gradient spatial style compositor layer player cli",
    );

    assert_eq!(report["backend"], "compositor");
    assert_eq!(report["recipeId"], "debugStyleCellPositionBindingV2Oracle");
    assert_eq!(report["compositionMode"], "native");
    assert_eq!(report["fallbackUsed"], false);
    assert_eq!(report["nativeLoweringSucceeded"], true);
    assert_eq!(report["compositionSpecSummary"]["shaderLayers"], 1);
...
#[test]
fn test_fnc_cli_lowers_wayfinding_node_shader_to_compositor_shader_layer_not_style_stage_json() {
    let report = player_cli_json(
        vec![
            str_arg("render-backend"),
            str_arg("--recipe"),
            recipe_path("shaders/compositions/shader_wayfinding_node_current_index_binding.json"),
            str_arg("--descriptor-pack"),
            descriptor_pack_path(),
            str_arg("--backend"),
            str_arg("compositor"),
        ...
        "render-backend native wayfinding-node shader compositor layer player cli",
    );
    ...
    assert_eq!(
        report["loweredEffectIds"],
        serde_json::json!(["shader.wayfindingNode"])
    );
}
...
#[test]
fn test_fnc_cli_lowers_radar_shader_to_compositor_shader_layer_not_style_stage_json() {
    for (recipe, recipe_id, effect_id, phase) in [
        (
        ...
        (
            ...
            "debugStyleSpatialEffectV2Oracle",
            "style.spatial",
            "dwell",
        ),
    ] {
        let report = player_cli_json(
            ...
            "render-backend native radar shader compositor layer player cli",
        );
        ...
        assert_eq!(
            report["compositionSpecSummary"]["shaderLayers"], 1,
            "{recipe}"
        );
        assert_eq!(
        ...
        assert_eq!(report["loweredEffectIds"], serde_json::json!([effect_id]));
    }
}

#[test]
fn test_fnc_cli_rejects_radar_shader_non_foreground_apply_to_json() {
    for value in ["background", "both"] {
        let temp_root = std::env::temp_dir().join(format!("tui-vfx-native-radar-{value}"));
        let _ = fs::remove_dir_all(&temp_root);
        fs::create_dir_all(&temp_root).expect("create temp unsupported radar fixture root");
        let recipe = unsupported_native_effect_shape_recipe(
            "shaders/primitives/shader_radar_sweep.json",
            Some(("applyTo", unsupported_native_enum_value(value))),
            None,
            None,
        );
        let recipe_path = temp_root.join(format!("radar_{value}.json"));
        fs::write(
            &recipe_path,
            serde_json::to_string_pretty(&recipe).expect("serialize unsupported radar recipe"),
        )
        .expect("write unsupported radar recipe");
        ...
        let output = run_player_cli(
            ...
            "render-backend native unsupported radar player cli",
        );
...
fn test_fnc_cli_native_style_rainbow_lowers_to_compositor_shader_json() {
    let report = player_cli_json(
        vec![
            str_arg("render-backend"),
            str_arg("--recipe"),
            recipe_path("styles/style_rainbow.json"),
            str_arg("--descriptor-pack"),
            descriptor_pack_path(),
            str_arg("--backend"),
            str_arg("compositor"),
        ...
        "render-backend native style rainbow player cli",
    );

    assert_eq!(report["backend"], "compositor");
    assert_eq!(report["recipeId"], "debugStyleRainbowV2Oracle");
    assert_eq!(report["fallbackUsed"], false);
    assert_eq!(report["nativeLoweringSucceeded"], true);
    assert_eq!(report["compositionSpecSummary"]["shaderLayers"], 1);
    assert_eq!(report["compositionSpecSummary"]["styleStages"], 0);
    assert_eq!(
        report["loweredEffectIds"],
        serde_json::json!(["style.rainbow"])
    );
    assert_eq!(
        report["styledCells"][0]["background"], "rgba(30,30,30,255)",
        "rainbow must preserve source background"
    );
    assert_ne!(
        report["styledCells"][0]["foreground"], "rgba(255,255,255,255)",
        "rainbow must rotate the source foreground"
    );
}
#[test]
