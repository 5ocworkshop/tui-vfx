...
/// Run the schema-readiness command and print its JSON report.
pub fn run_schema_readiness(options: CliOptions) -> Result<(), String> {
    ...
    let report = build_schema_readiness_report(
    ...
    println!(
        "{}",
        serde_json::to_string_pretty(&report).expect("schema readiness report serializes")
    );
    Ok(())
}
...
fn validate_options(options: &CliOptions) -> Result<(), String> {
    if !options.paths.is_empty() {
        return Err("schema-readiness does not accept recipe paths".to_string());
    }
    if options.family.is_none() && !options.recursive {
        return Err("schema-readiness requires --family or --recursive".to_string());
    }
    if options.width.is_some() || options.height.is_some() {
        return Err("schema-readiness does not accept frame size options".to_string());
    }
    if options.loop_t.is_some() {
        return Err("schema-readiness does not accept --loop-t".to_string());
    }
    Ok(())
}
