...
// <CLOG>0.2.1: PATCH — remove stale sampled-cell parameter from the merge seam.
// 0.2.0: MINOR — pass the sampled source role into role write policy execution.
// 0.1.1: PATCH — apply skipTransparentEmpty to the final write payload.</CLOG>

use tui_vfx_contract::{CellWritePolicy, RoleWritePolicy};
use tui_vfx_types::{Cell, Grid, RoleTag, SemanticScene};
...
pub(crate) fn merge_element_surface(
    ...
    match apply_cell_write_policy(cell_policy, &final_cell) {
        ...
        CellWriteDecision::Write => {
            ...
            apply_role_write_policy(destination, dest_x, dest_y, sampled_role, role_policy);
        }
    }
}
