...
// <WCTX>feat/cursor-scan: add a third cursor animation axis — CursorScan — that cycles the cursor glyph through bounded shapes (Pulse over the 1/8th-block ramp, HalfBlockBounce over ▀ █ ▄) while the cursor is parked. Grow-in still wins during its window; scan only fires in GrowInPhase::Visible.</WCTX>
// <CLOG>MINOR: new submodules cls_cursor_scan and fnc_cursor_scan_glyph; re-export CursorScan, ScanMode, and fnc_cursor_scan_glyph from the crate root.</CLOG>
...
pub use cls_cursor_scan::{CursorScan, ScanMode};
...
pub use fnc_cursor_scan_glyph::fnc_cursor_scan_glyph;
