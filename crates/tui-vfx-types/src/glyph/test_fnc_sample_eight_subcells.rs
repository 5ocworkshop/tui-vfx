...
// <CLOG>0.2.0: flatten the inner `mod tests` wrapper since the parent `#[path]` already wraps as `mod tests`; switch indexing loop to .iter().enumerate() to clear clippy::needless_range_loop.</CLOG>
...
fn test_sample_eight_returns_eight_scalars_in_dot_order() {
    ...
    for (i, &result) in results.iter().enumerate() {
        ...
        assert!(
            ...
            "results[{i}] = {result} expected {expected}"
        );
    }
}
