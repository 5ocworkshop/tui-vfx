...
// <CLOG>0.2.0: drop `, 0.0` time argument from every encoder call (encode_one/encode_subcell no longer take t); flatten the inner `mod tests` wrapper since the parent `#[path]` already wraps as `mod tests`.</CLOG>
...
fn test_braille_subcell_per_dot_threshold_worked_example() {
    ...
    let ch = encoder.encode_subcell(subcells, 0, 0);
...
fn test_braille_subcell_all_below_threshold_returns_empty() {
    ...
    let ch = encoder.encode_subcell([0.0f32; 8], 0, 0);
...
fn test_braille_subcell_all_above_threshold_returns_full() {
    ...
    let ch = encoder.encode_subcell([1.0f32; 8], 0, 0);
...
fn test_braille_subcell_nan_input_does_not_panic() {
    let encoder = GlyphEncoder::BrailleSubcell { threshold: 0.5 };
    let ch = encoder.encode_subcell([f32::NAN; 8], 0, 0);
...
fn test_braille_subcell_encode_one_averages_to_eighths_form() {
    ...
    assert_eq!(
        subcell_enc.encode_one(0.5, 0, 0),
        eighths_enc.encode_one(0.5, 0, 0),
...
fn test_braille_eighths_unrotated_byte_equivalent_to_subcell_light() {
    ...
    for &intensity in &[0.0f32, 0.125, 0.25, 0.5, 1.0] {
        let got = encoder.encode_one(intensity, 0, 0);
...
fn test_braille_eighths_rotated_byte_equivalent_to_subcell_light() {
    ...
    for &intensity in &[0.25f32, 0.5, 0.75, 1.0] {
        let got = encoder.encode_one(intensity, 3, 2);
    ...
    for &intensity in &[0.375f32, 0.625] {
        let got = encoder.encode_one(intensity, 7, 5);
...
fn test_block_horizontal_table_byte_for_byte() {
    ...
    for &(intensity, ch) in expected {
        assert_eq!(
            encoder.encode_one(intensity, 0, 0),
            ch,
...
fn test_ramp_basic() {
    ...
    assert_eq!(ramp.encode_one(0.0, 0, 0), 'a');
    assert_eq!(ramp.encode_one(1.0, 0, 0), 'd');
    // 0.5 * 3 = 1.5, round → 2 → 'c'
    assert_eq!(ramp.encode_one(0.5, 0, 0), 'c');
}

#[test]
fn test_ramp_empty_returns_space() {
    let ramp = GlyphEncoder::Ramp(Cow::Borrowed(&[]));
    assert_eq!(ramp.encode_one(0.5, 0, 0), ' ');
    assert_eq!(ramp.encode_one(0.0, 0, 0), ' ');
    assert_eq!(ramp.encode_one(1.0, 0, 0), ' ');
}

#[test]
fn test_ramp_clamp_below_zero() {
    let ramp = GlyphEncoder::Ramp(Cow::Borrowed(&['a', 'b', 'c']));
    assert_eq!(ramp.encode_one(-1.0, 0, 0), 'a');
}

#[test]
fn test_ramp_clamp_above_one() {
    let ramp = GlyphEncoder::Ramp(Cow::Borrowed(&['a', 'b', 'c']));
    assert_eq!(ramp.encode_one(2.0, 0, 0), 'c');
}
...
fn test_block_intensity_clamping() {
    ...
    assert_eq!(h.encode_one(-0.5, 0, 0), ' ');
    assert_eq!(h.encode_one(1.5, 0, 0), '█');
    assert_eq!(v.encode_one(-0.5, 0, 0), ' ');
    assert_eq!(v.encode_one(1.5, 0, 0), '█');
}
...
fn test_encoder_clone_works() {
    ...
    assert_eq!(
        enc.encode_subcell(subcells, 0, 0),
        cloned.encode_subcell(subcells, 0, 0)
    );
}
...
fn test_block_horizontal_encode_subcell_falls_back_to_average() {
    ...
    let subcell_ch = enc.encode_subcell([0.5f32; 8], 0, 0);
    let direct_ch = enc.encode_one(0.5, 0, 0);
    assert_eq!(
