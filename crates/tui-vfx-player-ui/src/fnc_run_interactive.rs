...
// <CLOG>0.3.0: MINOR — enable mouse capture and route clicks/drags to studio controls.
// 0.2.1: PATCH — pass full key events so Ctrl+Arrow drawer toggles are visible.</CLOG>
...
use crate::{
    PlayerUiApp, PlayerUiState, handle_player_ui_key_event, handle_player_ui_mouse_event,
    render_ratatui_ui,
};
        ...
        if event::poll(poll_timeout).map_err(|error| error.to_string())? {
            match event::read().map_err(|error| error.to_string())? {
                ...
                Event::Mouse(mouse) => {
                    ...
                    if !handle_player_ui_mouse_event(&mut app, mouse, area) {
                        break;
                    }
                }
                _ => {}
