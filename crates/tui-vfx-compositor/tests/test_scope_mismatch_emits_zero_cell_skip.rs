// <FILE>crates/tui-vfx-compositor/tests/test_scope_mismatch_emits_zero_cell_skip.rs</FILE> - <DESC>Regression test for the focused_row_btop bug class — a Role(Text) scope on an all-Background RoleMap must emit StageSkipped { ScopeMatchedZeroCells } with an honest role histogram</DESC>
...
use tui_vfx_compositor::pipeline::cls_composition_options::{CompositionOptions, ShaderWithRegion};
...
// <FILE>crates/tui-vfx-compositor/tests/test_scope_mismatch_emits_zero_cell_skip.rs</FILE> - <DESC>Regression test for focused_row_btop scope mismatch</DESC>
